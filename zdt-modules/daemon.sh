...
start_web_dashboard() {
    ...
    local port="${ZDT_WEB_PORT:-2000}"
    local host="${ZDT_WEB_BIND:-0.0.0.0}"
    ...
    _run_python_script "$web_script" --bind "$host" --port "$port"
}
...
start_cloudflared_tunnel() {
    ...
    echo -e "  ${CYAN}${ICO_ARROW} Mengecek instalasi cloudflared...${RESET}"
    if ! command -v cloudflared >/dev/null 2>&1; then
        echo -e "  ${YELLOW}${ICO_WARN} cloudflared belum terinstal. Mendownload secara otomatis...${RESET}"
        ...
        if [ -n "${TERMUX_VERSION:-}" ]; then
            echo -e "  ${YELLOW}${ICO_ARROW} Menginstal via pkg (Termux)...${RESET}"
            pkg install -y cloudflared
        elif [ -n "$dl_url" ]; then
            local bin_path="$HOME/.local/bin/cloudflared"
            mkdir -p "$HOME/.local/bin"
            echo -e "  ${CYAN}${ICO_ARROW} Mendownload dari: $dl_url${RESET}"
            if [[ "$dl_url" == *.tgz ]]; then
                curl -sL "$dl_url" | tar xz -C "$HOME/.local/bin" cloudflared
            else
                curl -sL "$dl_url" -o "$bin_path"
            fi
            chmod +x "$bin_path"
            export PATH="$HOME/.local/bin:$PATH"
            echo -e "  ${GREEN}${ICO_OK} Instalasi selesai!${RESET}"
        else
            echo -e "  ${RED}${ICO_FAIL} Sistem tidak didukung untuk instalasi otomatis.${RESET}"
            echo -e "  Silakan instal manual dari https://github.com/cloudflare/cloudflared/releases"
            return 1
        fi
    fi

    echo -e "  ${GREEN}${ICO_OK} cloudflared siap digunakan.${RESET}"
    
    local port="${WEB_PORT:-5000}"
    local host="${WEB_BIND:-127.0.0.1}"
    local url="http://${host}:${port}"

    echo -e "  ${YELLOW}${ICO_ARROW} Menjalankan Cloudflare Tunnel untuk $url...${RESET}"
    echo -e "  ${CYAN}Tunggu beberapa detik, link publik (https://....trycloudflare.com) akan muncul di bawah:${RESET}"
    echo -e "  ${DIM}Gunakan link tersebut beserta API Key kamu di aplikasi mobile ZDT.${RESET}"
    echo ""
    
    # Run cloudflared, passing output directly to the user
