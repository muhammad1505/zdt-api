...
_refresh_stats_cache() {
    ...
    # CPU & load & temp — refresh max every 3 seconds (independent timer)
...
print_header() {
    ...
    local is_mobile=0
    if [ "$cols" -lt 75 ] || [ "${RUNTIME_ENV:-}" = "termux" ] || [ "${FORCE_MOBILE:-0}" -eq 1 ]; then
        is_mobile=1
    fi
    local tlc="╔" trc="╗" sep_l="╠" sep_r="╣" vc="║" hc="═"
    if [ "$is_mobile" -eq 1 ]; then
        tlc="╭" trc="╮" sep_l="├" sep_r="┤" vc="│" hc="─"
    fi
    ...
    # Top border
    echo -e "  ${_ZDT_BORDER_PRIMARY}${tlc}$(_repeat_char "$hc" $width)${trc}${RESET}"
    ...
    echo -e "  ${_ZDT_BORDER_PRIMARY}${vc}${RESET}${title_pad}${_ZDT_BORDER_PRIMARY}${vc}${RESET}"

    # Separator between caption and content
    echo -e "  ${_ZDT_BORDER_PRIMARY}${sep_l}$(_repeat_char "$hc" $width)${sep_r}${RESET}"
    echo ""
}
...
# Left panel full ═ separator, right panel normal content row
_draw_split_sep_left() {
    local lw=$1 rtxt="$2" rw=$3
    local rp=$(_pad_str "$rtxt" "$rw")
    echo -e "  ${_ZDT_BORDER_PRIMARY}╠$(_repeat_char '═' $lw)╣${RESET}${rp}${_ZDT_BORDER_PRIMARY}║${RESET}"
}

# Left panel normal content row, right panel full ═ separator
_draw_split_sep_right() {
    local ltxt="$1" lw=$2 rw=$3
    local lp=$(_pad_str "$ltxt" "$lw")
    echo -e "  ${_ZDT_BORDER_PRIMARY}║${RESET}${lp}${_ZDT_BORDER_PRIMARY}╠$(_repeat_char '═' $rw)╣${RESET}"
}
...
_get_box_chars() {
    local cols=$(tput cols 2>/dev/null || echo 80)
    _BOX_TLC="╔" _BOX_TRC="╗" _BOX_BLC="╚" _BOX_BRC="╝" _BOX_VC="║" _BOX_HC="═" _BOX_SEPL="╠" _BOX_SEPR="╣"
    if [ "$cols" -lt 75 ] || [ "${RUNTIME_ENV:-}" = "termux" ] || [ "${FORCE_MOBILE:-0}" -eq 1 ]; then
        _BOX_TLC="╭" _BOX_TRC="╮" _BOX_BLC="╰" _BOX_BRC="╯" _BOX_VC="│" _BOX_HC="─" _BOX_SEPL="├" _BOX_SEPR="┤"
    fi
}
...
_draw_box_top() {
    local width=$1
    _get_box_chars
    echo -e "  ${_ZDT_BORDER_PRIMARY}${_BOX_TLC}$(_repeat_char "$_BOX_HC" $width)${_BOX_TRC}${RESET}"
}
...
_draw_box_row() {
    ...
    _get_box_chars
    echo -e "  ${_ZDT_BORDER_PRIMARY}${_BOX_VC}${RESET}${pad}${_ZDT_BORDER_PRIMARY}${_BOX_VC}${RESET}"
}

_draw_box_sep() {
    local width=$1
    _get_box_chars
    echo -e "  ${_ZDT_BORDER_PRIMARY}${_BOX_SEPL}$(_repeat_char "$_BOX_HC" $width)${_BOX_SEPR}${RESET}"
}

_draw_box_bottom() {
    local width=$1
    _get_box_chars
    echo -e "  ${_ZDT_BORDER_PRIMARY}${_BOX_BLC}$(_repeat_char "$_BOX_HC" $width)${_BOX_BRC}${RESET}"
}
...
_pause() {
    ...
    local tlc="╔" trc="╗" blc="╚" brc="╝" vc="║" hc="═"
    if [ "$is_mobile" -eq 1 ]; then
        tlc="╭" trc="╮" blc="╰" brc="╯" vc="│" hc="─"
    fi

    echo -e "  ${_ZDT_BORDER_PRIMARY}${tlc}$(_repeat_char "$hc" $_pw)${trc}${RESET}"
    echo -e "  ${_ZDT_BORDER_PRIMARY}${vc}${RESET}${_pp}${_ZDT_BORDER_PRIMARY}${vc}${RESET}"
    echo -e "  ${_ZDT_BORDER_PRIMARY}${blc}$(_repeat_char "$hc" $_pw)${brc}${RESET}"
    read -s -r -n 1 2>/dev/null || read -r -n 1 2>/dev/null || true
    echo ""
}
