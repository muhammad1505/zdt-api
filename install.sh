...
install_system_packages() {
    ...
    $PKG_INSTALL redis-server redis-tools 2>/dev/null || warn "Redis installation failed (optional — fallback to in-memory rate limiting)"
...
generate_config() {
    ...
    # Copy config.env.example as template (more complete than inline template)
    if [ -f "$ZDT_API_DIR/config.env.example" ]; then
        cp "$ZDT_API_DIR/config.env.example" "$config_file"
        sed -i "s|TARGET_DIR=.*|TARGET_DIR=$USER_HOME/Music/ZDT_Downloads|" "$config_file"
        sed -i "s|JWT_SECRET=.*|JWT_SECRET=$jwt_secret|" "$config_file"
        sed -i "s|ZDT_WEB_PASS=.*|ZDT_WEB_PASS=$web_pass|" "$config_file"
    else
...
# Redis (optional — for multi-worker rate limiting)
REDIS_URL=
CONFEOF
    fi
...
" 2>/dev/null &&    ok "Database initialized" || warn "Database init failed (will be auto-created on first run)"
    ...
    # Run old DB migration if available
    if [ -f "$ZDT_API_DIR/migrate_zdt_db.py" ]; then
        info "Checking for old database to migrate..."
        $python_cmd "$ZDT_API_DIR/migrate_zdt_db.py" --dry-run 2>/dev/null && {
            info "Old database found! Run migration: python3 migrate_zdt_db.py"
        } || true
    fi
}
