...
def _print_credentials():
    ...
    if conf_user and conf_pass:
        print(f"  🔐 Web Dashboard: {conf_user} / {conf_pass}")
        print(f"  🔑 API Key: {conf_pass}")
    else:
        print(f"  🔐 Web Dashboard: admin / (lihat output terminal di atas)")
    ...
    def decorated(*args, **kwargs):
        ...
        if os.path.exists(config_file):
            with open(config_file, 'r') as cf:
                for line in cf:
                    line = line.strip()
                    if line.startswith("ZDT_WEB_PASS="):
                        conf_pass = line.split("=", 1)[1].strip('"\'')
                        
        if api_key and api_key == conf_pass:
            return f(*args, **kwargs)
